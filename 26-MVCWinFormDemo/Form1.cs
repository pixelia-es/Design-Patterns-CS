...
namespace _26_MVCWinFormDemo
{
    public partial class StudentForm : Form
    {
        ...
        {
            //We will no add an empty student name
            if (addStudentTextBox.Text != String.Empty)
            {
                ...
                //Now display back to GUI. This value is comming through the controller
                showStudentsListView.Items.Clear();
                studentController.GetEnrolledStudents();
        ...
        {
            //We can select only on item at a time
            if(showStudentsListView.SelectedItems.Count==1)
            {
                ...
                List<string> updatedStudentList = new List<string>();
                //Remove the student name and get the new student list throught controller
                updatedStudentList = studentController.RemoveStudent(studentName);
                //Noew display back to GUI. This value is comming through the controller
                showStudentsListView.Items.Clear();
                studentController.GetEnrolledStudents();
            }
    ...
    public class StudentController : IController
    {
        ...
        //Sending a request to modelo to remove a student from the student list
    ...
    {
        private List<string> enrolledStudents = new List<string> { "Amit", "Jhon", "Sam"};
        ...
        //Removing a student fro model (student list)
